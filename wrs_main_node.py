...
class WrsMainController(object):
    ...
    def __init__(self):
        ...
        self.target_category = self.load_json(self.get_path(["config", "target_category.json"]))
        self.category_place = self.load_json(self.get_path(["config", "category_place.json"]))
        # ROS通信関連の初期化
    ...
    def get_most_graspable_obj(cls, obj_list):
        ...
        print(sorted(extracted, key=lambda x: x["y"], reverse=True))
        for obj_info in sorted(extracted, key=lambda x: x["y"], reverse=True):
            obj = obj_info["bbox"]
            print(obj.y)
    ...
    def exec_graspable_method(self, grasp_pos, label=""):
        ...
        elif (label == "cracker_box"):
            method = self.grasp_cracker_box
        else:
    ...
    def deliver_to_target(self, target_obj, target_person):
        ...
        if target_person == "left":
            ...
            human_name = "person_b"
        self.goto_name(human_name)    # TODO: 配達先が固定されているので修正
    ...
    def run(self):
        ...
        self.pull_out_trofast(0.172, -0.29, 0.28, -90, 100, 0)
        self.pull_out_trofast(0.478, -0.31, 0.29, -90, 100, 0)
        self.change_pose("all_neutral")
        self.execute_task1()
        self.execute_task2a()
        self.execute_task2b()
